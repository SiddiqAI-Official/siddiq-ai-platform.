...
export default function SiddiqAI() {
  ...
  const startNewProject = () => {
    if(confirm("Clear everything and start new?")) {
      localStorage.clear();
  ...
  const saveToCloud = async () => {
    ...
    else alert("Project Saved to Cloud Successfully! ☁️");
  };
  ...
  const generateWebsite = async () => {
    ...
    try {
      ...
      setPrompt('');
      setView('preview');
    } catch (err) { alert("AI Build Error! Please check your prompt."); }
    setLoading(false);
  };

  const iframeContent = useMemo(() => {
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" rel="stylesheet"><style>body{margin:0;padding:0;overflow-x:hidden;font-family:sans-serif;}</style><script>window.onclick=function(e){if(e.target.tagName==="A")e.preventDefault();};</script></head><body>${generatedCode || '<div style="display:flex;align-items:center;justify-content:center;height:100vh;color:#ccc;text-transform:uppercase;letter-spacing:5px;font-size:10px;font-weight:bold;">Siddiq AI Master v15 Ready</div>'}<script>async function loadImgs(){const imgs=document.querySelectorAll("img");for(let img of imgs){const alt=img.alt||"luxury";img.src="https://placehold.co/800x600?text=Generating+DALL-E+3+Image...";try{const res=await fetch("/api/image",{method:"POST",body:JSON.stringify({prompt:alt}),headers:{"Content-Type":"application/json"}});const data=await res.json();if(data.url)img.src=data.url;}catch(e){console.error(e);}}}window.onload=loadImgs;</script></body></html>`;
  }, [generatedCode]);
      ...
      <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-white/10 flex flex-col bg-[#0a0a0a] z-30 shadow-xl">
        <div className="p-4 md:p-6 border-b border-white/5 flex justify-between items-center">
          ...
          <div className="flex gap-4 text-gray-500">
            <button onClick={startNewProject} title="New"><i className="fas fa-plus-circle hover:text-white"></i></button>
            <button onClick={saveToCloud} title="Save"><i className="fas fa-cloud hover:text-blue-400"></i></button>
            <button onClick={() => {localStorage.removeItem('siddiq_access'); window.location.reload();}}><i className="fas fa-power-off hover:text-red-500"></i></button>
          </div>
        </div>
        ...
        <div className="p-4 bg-[#0a0a0a] border-t border-white/5 space-y-3">
          <textarea className="w-full p-4 bg-white/5 border border-white/10 rounded-xl focus:border-blue-500 outline-none h-24 text-sm text-white resize-none shadow-inner" placeholder="What to build next?" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
          <button onClick={generateWebsite} className="w-full bg-blue-600 hover:bg-blue-500 p-3 rounded-xl font-bold shadow-lg disabled:opacity-50 text-xs tracking-widest transition-all" disabled={loading}>
            {loading ? <i className="fas fa-spinner fa-spin"></i> : 'GENERATE MASTER DESIGN'}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-[#f1f5f9] overflow-hidden">
        ...
        <div className="flex-1 overflow-auto flex justify-center items-start md:p-8 p-2 bg-[#e2e8f0]/50 text-black">
          <div style={{ width: previewSize === 'desktop' ? '100%' : previewSize === 'tablet' ? '768px' : '375px', height: previewSize === 'desktop' ? '100%' : '667px' }} className="transition-all duration-500 shadow-2xl bg-white overflow-hidden rounded-2xl border border-gray-300 mx-auto relative">
            {view === 'preview' ? (
              <iframe srcDoc={iframeContent} className="w-full h-full border-none bg-white" />
            ) : (
