...
export default function SiddiqAI() {
  ...
  const saveToCloud = async () => {
    const { error } = await supabase.from('projects').insert([
      { name: 'Design ' + new Date().toLocaleTimeString(), code: generatedCode, history: messages }
    ]);
    ...
    return (
      <div className="h-screen bg-black flex items-center justify-center p-6 text-white font-sans text-center">
        <div className="w-full max-w-sm bg-gray-900 border border-white/10 p-8 rounded-[2rem] shadow-2xl">
          ...
          <input type="password" placeholder="ACCESS CODE" className="w-full bg-black border border-gray-800 p-4 rounded-xl outline-none focus:border-blue-600 mb-4 text-white text-center font-bold tracking-widest" value={passInput} onChange={(e) => setPassInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} />
      ...
      <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-white/10 flex flex-col bg-[#0a0a0a] z-30 shadow-xl">
        <div className="p-4 md:p-6 border-b border-white/5 flex justify-between items-center">
          <div>
            ...
            <p className="text-[8px] text-gray-500 font-bold uppercase tracking-[0.3em]">v3.1 Stable</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((m:any, i:number) => (
            <div key={i} className={`p-3 rounded-2xl max-w-[85%] text-xs ${m.role === 'user' ? 'bg-blue-600 ml-auto' : 'bg-white/5 mr-auto text-gray-400'}`}>
              {m.content.length > 100 ? m.content.substring(0, 50) + '...' : m.content}
            </div>
          ))}
        </div>

        <div className="p-4 bg-[#0a0a0a] border-t border-white/5 space-y-3">
          <textarea className="w-full p-4 bg-white/5 border border-white/10 rounded-xl focus:border-blue-500 outline-none h-24 text-sm text-white resize-none shadow-inner" placeholder="Describe your vision..." value={prompt} onChange={(e) => setPrompt(e.target.value)} />
      ...
      <div className="flex-1 flex flex-col bg-[#f1f5f9] overflow-hidden">
        <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 z-20 shadow-sm">
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg gap-1 border border-gray-200">
            <button onClick={() => setPreviewSize('desktop')} className={`px-4 py-1.5 rounded-lg text-xs ${previewSize === 'desktop' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}><i className="fas fa-desktop"></i></button>
            <button onClick={() => setPreviewSize('tablet')} className={`px-4 py-1.5 rounded-lg text-xs ${previewSize === 'tablet' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}><i className="fas fa-tablet-alt"></i></button>
            <button onClick={() => setPreviewSize('mobile')} className={`px-4 py-1.5 rounded-lg text-xs ${previewSize === 'mobile' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}><i className="fas fa-mobile-alt"></i></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto flex justify-center items-start md:p-8 p-2 bg-[#e2e8f0]/50">
          <div style={{ width: previewSize === 'desktop' ? '100%' : previewSize === 'tablet' ? '768px' : '375px', height: previewSize === 'desktop' ? '100%' : '667px' }} className="transition-all duration-500 shadow-2xl bg-white overflow-hidden rounded-2xl border border-gray-300 mx-auto">
            {view === 'preview' ? (
              <iframe srcDoc={`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" rel="stylesheet"><style>body{margin:0;padding:0;overflow-x:hidden;font-family:sans-serif;}</style><script>window.onclick=function(e){if(e.target.tagName==="A")e.preventDefault();};</script></head><body>${generatedCode || '<div style="display:flex;align-items:center;justify-content:center;height:100vh;color:#ccc;text-transform:uppercase;letter-spacing:5px;font-size:10px;">Ready for command</div>'}</body></html>`} className="w-full h-full border-none" />
            ) : (
