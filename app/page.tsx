...
export default function SiddiqAI() {
  ...
  const saveToCloud = async () => {
    const { error } = await supabase.from('projects').insert([
      { name: 'Siddiq Design ' + new Date().toLocaleDateString(), code: generatedCode, history: messages }
    ]);
    ...
    return (
      <div className="h-screen bg-black flex items-center justify-center p-6 text-white font-sans">
        <div className="w-full max-w-sm bg-gray-900 border border-white/10 p-10 rounded-[2.5rem] shadow-2xl text-center">
          ...
          <input type="password" placeholder="ACCESS CODE" className="w-full bg-black border border-gray-800 p-4 rounded-xl outline-none focus:border-blue-600 mb-4 text-center font-bold tracking-[0.5em] text-white" value={passInput} onChange={(e) => setPassInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} />
      ...
      <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-white/10 flex flex-col bg-[#0a0a0a] z-30 shadow-xl">
        <div className="p-4 md:p-6 border-b border-white/5 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            {generatedCode && <button onClick={saveToCloud} className="text-gray-500 hover:text-blue-400" title="Save to Cloud"><i className="fas fa-cloud"></i></button>}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black/20">
          {messages.map((m, i) => (
            <div key={i} className={`p-3 rounded-2xl max-w-[85%] text-xs leading-relaxed ${m.role === 'user' ? 'bg-blue-600 ml-auto text-white' : 'bg-white/5 mr-auto text-gray-400 border border-white/5'}`}>
              {m.content.length > 200 ? m.content.substring(0, 100) + '... [Code Generated]' : m.content}
            </div>
          ))}
          <div ref={chatEndRef} />
        </div>
