  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#050505] text-white overflow-hidden font-sans">
      ...
      <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-white/10 flex flex-col bg-[#0a0a0a] z-30 shadow-xl p-6">
        <h1 className="text-xl font-black text-blue-500 mb-6 italic">SIDDIQ AI <span className="text-[10px] text-gray-600">v4.0</span></h1>
        <textarea className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl focus:border-blue-500 outline-none h-48 text-sm text-white transition-all mb-4" placeholder="E.g. Luxury villa with a pool..." value={prompt} onChange={(e) => setPrompt(e.target.value)} />
        <button onClick={generateWebsite} className="w-full bg-blue-600 hover:bg-blue-500 p-4 rounded-2xl font-black text-xs tracking-widest disabled:opacity-50" disabled={loading}>
          {loading ? 'GENERATING AI IMAGES...' : 'BUILD WITH REAL PHOTOS'}
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-[#f1f5f9] overflow-hidden p-4 md:p-12">
        <div className="w-full h-full bg-white shadow-2xl rounded-3xl overflow-hidden border border-gray-200">
          {generatedCode ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-gray-300 uppercase tracking-widest text-xs font-bold">Waiting for your vision...</div>
          )}
        </div>
      </div>
    </div>
