...
export default function SiddiqAI() {
  ...
  const startNewProject = () => {
    if(confirm("Clear and start new?")) {
      localStorage.clear();
    ...
    return (
      <div className="h-screen bg-black flex items-center justify-center p-6 text-white text-center">
        <div className="w-full max-w-sm bg-gray-900 border border-white/10 p-10 rounded-[2.5rem] shadow-2xl">
          ...
          <input type="password" placeholder="CODE" className="w-full bg-black border border-gray-800 p-4 rounded-xl outline-none focus:border-blue-600 mb-4 text-white text-center font-bold" value={passInput} onChange={(e) => setPassInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} />
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#050505] text-white overflow-hidden font-sans">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black/20">
          {messages.filter(m => m.role === 'user').map((m, i) => (
            <div key={i} className="p-3 rounded-xl bg-blue-600/20 border border-blue-600/30 text-[10px] text-blue-100 flex items-start gap-2">
              <i className="fas fa-comment-dots mt-1 opacity-50"></i> {m.content.substring(0, 100)}...
            </div>
          ))}
          <div ref={chatEndRef} />
        </div>
      ...
      <div className="flex-1 flex flex-col bg-[#f1f5f9] overflow-hidden">
        <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 z-20 shadow-sm text-black">
          <div className="flex gap-4">
        ...
        <div className="flex-1 overflow-auto flex justify-center items-start md:p-8 p-2 bg-[#e2e8f0]/50 text-black">
          <div style={{ width: previewSize === 'desktop' ? '100%' : previewSize === 'tablet' ? '768px' : '375px', height: previewSize === 'desktop' ? '100%' : '667px' }} className="transition-all duration-500 shadow-2xl bg-white overflow-hidden rounded-2xl border border-gray-300 mx-auto">
            {view === 'preview' ? (
              <iframe srcDoc={`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" rel="stylesheet"><style>body{margin:0;padding:0;overflow-x:hidden;font-family:sans-serif;}</style><script>window.onclick=function(e){if(e.target.tagName==="A")e.preventDefault();};</script></head><body>${generatedCode}<script>async function loadAI(){const imgs=document.querySelectorAll("img");for(let img of imgs){const alt=img.alt||"luxury property";img.src="https://placehold.co/800x600?text=Generating+DALL-E+3+Photo...";try{const res=await fetch("/api/image",{method:"POST",body:JSON.stringify({prompt:alt}),headers:{"Content-Type":"application/json"}});const data=await res.json();if(data.url)img.src=data.url;}catch(e){console.error(e);}}}window.onload=loadAI;</script></body></html>`} className="w-full h-full border-none bg-white" />
            ) : (
