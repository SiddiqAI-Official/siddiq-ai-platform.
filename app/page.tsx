...
export default function SiddiqAI() {
  ...
  const iframeContent = useMemo(() => {
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" rel="stylesheet"><style>body{margin:0;padding:0;overflow-x:hidden;font-family:sans-serif;}</style><script>window.onclick=function(e){if(e.target.tagName==="A"){e.preventDefault();e.stopPropagation();}};</script></head><body>${generatedCode || '<div style="display:flex;align-items:center;justify-content:center;height:100vh;color:#ccc;text-transform:uppercase;letter-spacing:5px;font-size:10px;font-weight:bold;">Siddiq AI Titan v11 Ready</div>'}</body></html>`;
  }, [generatedCode]);
