...
export default function SiddiqAI() {
  ...
  const startNewProject = () => {
    if(confirm("New project? History will be cleared.")) {
      localStorage.clear();
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#050505] text-white overflow-hidden font-sans">
      ...
      <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-white/10 flex flex-col bg-[#0a0a0a] z-30 shadow-xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black/20">
          {messages.filter(m => m.role === 'user').map((m:any, i:number) => (
        ...
        <div className="p-4 bg-[#0a0a0a] border-t border-white/5 space-y-3">
          ...
          <button onClick={generateWebsite} className="w-full bg-blue-600 hover:bg-blue-500 p-3 rounded-xl font-bold text-xs" disabled={loading}>
            {loading ? <i className="fas fa-spinner fa-spin"></i> : 'UPDATE DESIGN'}
          </button>
        </div>
      </div>
      <div className="flex-1 flex flex-col bg-[#f1f5f9] overflow-hidden text-black font-bold">
