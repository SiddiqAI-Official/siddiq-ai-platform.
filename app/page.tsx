    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#050505] text-white overflow-hidden font-sans">
      ...
      <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-white/10 flex flex-col bg-[#0a0a0a] z-30 shadow-xl">
        ...
        <div className="p-4 bg-[#0a0a0a] border-t border-white/5 space-y-3">
          <textarea className="w-full p-4 bg-white/5 border border-white/10 rounded-xl focus:border-blue-500 outline-none h-24 text-sm text-white resize-none" placeholder="What should I build?" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
          <button onClick={generateWebsite} className="w-full bg-blue-600 hover:bg-blue-500 p-3 rounded-xl font-bold text-xs" disabled={loading}>{loading ? <i className="fas fa-spinner fa-spin"></i> : 'GENERATE'}</button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-[#f1f5f9] overflow-hidden">
        <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 z-20 shadow-sm text-black">
          <div className="flex gap-4">
            <button onClick={() => setView('preview')} className={`text-[10px] font-black uppercase ${view === 'preview' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400'}`}>Canvas</button>
            <button onClick={() => setView('code')} className={`text-[10px] font-black uppercase ${view === 'code' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400'}`}>Source</button>
          </div>
        ...
        <div className="flex-1 overflow-auto flex justify-center items-start md:p-8 p-2">
          <div style={{ width: previewSize === 'desktop' ? '100%' : previewSize === 'tablet' ? '768px' : '375px', height: previewSize === 'desktop' ? '100%' : '667px' }} className="transition-all duration-500 shadow-2xl bg-white overflow-hidden rounded-2xl border border-gray-300 mx-auto">
            {view === 'preview' ? (
              <iframe srcDoc={`<!DOCTYPE html><html><head><meta charset="UTF-8"><script src="https://cdn.tailwindcss.com"></script></head><body style="margin:0;">${generatedCode}<script>async function loadImgs(){const imgs=document.querySelectorAll("img");for(const img of imgs){const alt=img.alt||"luxury";img.src="https://placehold.co/800x600?text=Generating+DALL-E+3...";try{const res=await fetch("/api/image",{method:"POST",body:JSON.stringify({prompt:alt}),headers:{"Content-Type":"application/json"}});const data=await res.json();if(data.url)img.src=data.url;}catch(e){console.error(e);}}}window.onload=loadImgs;</script></body></html>`} className="w-full h-full border-none bg-white" />
            ) : (
