...
export default function SiddiqAI() {
  ...
  const startNewProject = () => {
    if(confirm("New project? All history will be cleared.")) {
      localStorage.clear();
  ...
  const saveToCloud = async () => {
    const { error } = await supabase.from('projects').insert([{ name: 'Siddiq Master Design', code: generatedCode, history: messages }]);
    if (error) alert("DB Error: " + error.message);
    else alert("Saved to Cloud Successfully! ☁️");
  };
  ...
  const iframeContent = useMemo(() => {
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" rel="stylesheet"><style>body{margin:0;padding:0;overflow-x:hidden;font-family:sans-serif;}</style><script>window.onclick=function(e){if(e.target.tagName==="A"){e.preventDefault();}};</script></head><body>${generatedCode || '<div style="display:flex;align-items:center;justify-content:center;height:100vh;color:#ccc;text-transform:uppercase;letter-spacing:5px;font-size:10px;font-weight:bold;">Siddiq Master Ready</div>'}<script>async function loadAI(){const imgs=document.querySelectorAll("img");for(let img of imgs){const alt=img.alt||"luxury property";img.src="https://placehold.co/800x600?text=Generating+DALL-E+3+Image...";try{const res=await fetch("/api/image",{method:"POST",body:JSON.stringify({prompt:alt}),headers:{"Content-Type":"application/json"}});const data=await res.json();if(data.url)img.src=data.url;}catch(e){console.error(e);}}}window.onload=loadAI;</script></body></html>`;
  }, [generatedCode]);
      ...
      <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-white/10 flex flex-col bg-[#0a0a0a] z-30 shadow-xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black/20">
          {messages.filter(m => m.role === 'user').map((m, i) => (
            <div key={i} className="p-3 rounded-xl bg-blue-600/20 border border-blue-600/30 text-[10px] text-blue-100"><i className="fas fa-comment-dots mr-2 opacity-50"></i> {m.content}</div>
          ))}
        </div>

        <div className="p-4 bg-[#0a0a0a] border-t border-white/5 space-y-3 text-left">
          <textarea className="w-full p-4 bg-white/5 border border-white/10 rounded-xl focus:border-blue-500 outline-none h-24 text-sm text-white resize-none" placeholder="What to build?" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
          <button onClick={generateWebsite} className="w-full bg-blue-600 hover:bg-blue-500 p-3 rounded-xl font-bold shadow-lg disabled:opacity-50 text-xs uppercase" disabled={loading}>
