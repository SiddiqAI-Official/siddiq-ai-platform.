...
export default function SiddiqAI() {
  ...
  const saveToCloud = async () => {
    const { error } = await supabase.from('projects').insert([{ name: 'Siddiq Design', code: generatedCode, history: messages }]);
  ...
  const generateWebsite = async () => {
    ...
    try {
      const res = await fetch('/api/chat', { method: 'POST', body: JSON.stringify({ messages: newMsgs }), headers: { 'Content-Type': 'application/json' } });
      const data = await res.json();
      let html = data.code.replace(/```html|```/g, '').trim();
      
      setGeneratedCode(html);
      setMessages([...newMsgs, { role: 'assistant', content: html }]);
      localStorage.setItem('siddiq_code', html);
      localStorage.setItem('siddiq_msgs', JSON.stringify([...newMsgs, { role: 'assistant', content: html }]));
      setPrompt('');

      // PROCESS IMAGES AFTER HTML GENERATION
    ...
    } catch (err) { alert("AI Engine Busy. Try again."); }
    setLoading(false);
  };
    ...
    return (
      <div className="h-screen bg-black flex items-center justify-center p-6 font-sans">
        <div className="w-full max-w-sm bg-gray-900 border border-white/10 p-10 rounded-[2.5rem] shadow-2xl text-center">
          ...
          <button onClick={handleLogin} className="w-full bg-blue-600 p-4 rounded-xl font-bold uppercase text-white">Unlock</button>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#050505] text-white overflow-hidden font-sans">
      ...
      <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-white/10 flex flex-col bg-[#0a0a0a] z-30 shadow-xl">
        <div className="p-4 md:p-6 border-b border-white/5 flex justify-between items-center">
          ...
          <div className="flex gap-4">
             <button onClick={startNewProject} title="New"><i className="fas fa-plus-circle text-gray-500 hover:text-white"></i></button>
             <button onClick={saveToCloud} title="Save"><i className="fas fa-cloud text-gray-500 hover:text-blue-400"></i></button>
             <button onClick={() => {localStorage.removeItem('siddiq_access'); window.location.reload();}}><i className="fas fa-power-off text-gray-500 hover:text-red-500"></i></button>
          </div>
        </div>
        ...
        <div className="p-4 bg-[#0a0a0a] border-t border-white/5 space-y-3">
          <textarea className="w-full p-4 bg-white/5 border border-white/10 rounded-xl focus:border-blue-500 outline-none h-24 text-sm text-white resize-none" placeholder="Build your vision..." value={prompt} onChange={(e) => setPrompt(e.target.value)} />
          <button onClick={generateWebsite} className="w-full bg-blue-600 hover:bg-blue-500 p-3 rounded-xl font-bold shadow-lg disabled:opacity-50 text-xs tracking-widest" disabled={loading}>
            {loading ? <i className="fas fa-spinner fa-spin"></i> : 'GENERATE GLOBAL TITAN'}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-[#f1f5f9] overflow-hidden">
        <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 z-20 shadow-sm text-black font-bold">
          <div className="flex gap-4">
            <button onClick={() => setView('preview')} className={`text-[10px] uppercase ${view === 'preview' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400'}`}>Canvas</button>
            <button onClick={() => setView('code')} className={`text-[10px] uppercase ${view === 'code' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400'}`}>Source</button>
          </div>
          <div className="flex bg-gray-100 p-1 rounded-lg gap-1 border border-gray-200">
            ...
            <button onClick={() => setPreviewSize('tablet')} className={`p-1.5 px-3 rounded-md text-xs ${previewSize === 'tablet' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400'}`}><i className="fas fa-tablet-alt"></i></button>
        ...
        <div className="flex-1 overflow-auto flex justify-center items-start md:p-8 p-2 bg-[#e2e8f0]/50 text-black">
          <div style={{ width: previewSize === 'desktop' ? '100%' : previewSize === 'tablet' ? '768px' : '375px', height: previewSize === 'desktop' ? '100%' : '667px' }} className="transition-all duration-500 shadow-2xl bg-white overflow-hidden rounded-2xl border border-gray-300 mx-auto">
            {view === 'preview' ? (
              <iframe srcDoc={`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" rel="stylesheet"><style>body{margin:0;padding:0;overflow-x:hidden;font-family:sans-serif;}</style><script>window.onclick=function(e){if(e.target.tagName==="A"){e.preventDefault();}};</script></head><body>${generatedCode || '<div style="display:flex;align-items:center;justify-content:center;height:100vh;color:#ccc;text-transform:uppercase;letter-spacing:5px;font-size:10px;font-weight:bold;">Siddiq AI v14.0 Ready</div>'}</body></html>`} className="w-full h-full border-none bg-white" />
            ) : (
